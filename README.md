...
`ducktools.classbuilder` has much better import time performance by avoiding heavy stdlib imports.
...
It also defers the heavy `exec` calls that make up the majority of dataclasses' class construction time. `prefab_eval` in this case includes all of these calls.
